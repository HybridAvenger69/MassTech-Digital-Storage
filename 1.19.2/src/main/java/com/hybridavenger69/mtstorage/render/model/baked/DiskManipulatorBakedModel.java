...
import com.hybridavenger69.mtstorage.block.DiskManipulatorBlock;
...
public class DiskManipulatorBakedModel extends BakedModelWrapper<BakedModel> {
    ...
    private final LoadingCache<CacheKey, List<BakedQuad>> cache = CacheBuilder.newBuilder().build(new CacheLoader<CacheKey, List<BakedQuad>>() {
        ...
        public List<BakedQuad> load(CacheKey key) {
            Direction facing = key.state.getValue(MSBlocks.DISK_MANIPULATOR.get(ColorMap.DEFAULT_COLOR).get().getDirection().getProperty());
    ...
    public List<BakedQuad> getQuads(@Nullable final BlockState state,
        ...
        var color = MSBlocks.DISK_MANIPULATOR.getColorFromObject((DiskManipulatorBlock) state.getBlock());
