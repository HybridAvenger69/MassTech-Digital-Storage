...
public class FilterItem extends Item {
    ...
    public FilterItem() {
        super(new Item.Properties().tab(MS.CREATIVE_MODE_TAB).stacksTo(1));
    }
        ...
        if (!level.isClientSide) {
            ...
            player.openMenu(new MenuProvider() {
                ...
                public Component getDisplayName() {
                    return Component.translatable("gui.mtstorage.filter");
                }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("sidebutton.mtstorage.mode." + (getMode(stack) == IFilter.MODE_WHITELIST ? "whitelist" : "blacklist")).setStyle(Styles.YELLOW));

        if (isModFilter(stack)) {
            tooltip.add(Component.translatable("gui.mtstorage.filter.mod_filter").setStyle(Styles.BLUE));
        }
