...
import com.hybridavenger69.mtstorage.container.transfer.TransferManager;
import com.hybridavenger69.mtstorage.network.FluidFilterSlotUpdateMessage;
import com.hybridavenger69.mtstorage.blockentity.BaseBlockEntity;
import com.hybridavenger69.mtstorage.blockentity.data.BlockEntitySynchronizationWatcher;
        ...
        if (this.getPlayer() instanceof ServerPlayer) {
            for (int i = 0; i < this.fluidSlots.size(); ++i) {
                ...
                if (!API.instance().getComparer().isEqual(cached, actual, IComparer.COMPARE_QUANTITY | IComparer.COMPARE_NBT)) {
                    ...
                    MS.NETWORK_HANDLER.sendTo((ServerPlayer) getPlayer(), new FluidFilterSlotUpdateMessage(((Slot) slot).index, actual));
                }
            }
        }
    }
