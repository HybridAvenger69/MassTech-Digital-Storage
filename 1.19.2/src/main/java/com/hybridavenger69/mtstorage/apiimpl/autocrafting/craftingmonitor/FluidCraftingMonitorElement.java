...
public class FluidCraftingMonitorElement implements ICraftingMonitorElement {
    ...
    public void draw(PoseStack poseStack, int x, int y, IElementDrawers drawers) {
        ...
        if (stored > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.stored", API.instance().getQuantityFormatter().formatInBucketForm(stored)));

            yy += 7;
        }

        if (missing > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.missing", API.instance().getQuantityFormatter().formatInBucketForm(missing)));

            yy += 7;
        }

        if (processing > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.processing", API.instance().getQuantityFormatter().formatInBucketForm(processing)));

            yy += 7;
        }

        if (scheduled > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.scheduled", API.instance().getQuantityFormatter().formatInBucketForm(scheduled)));

            yy += 7;
        }

        if (crafting > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.crafting", API.instance().getQuantityFormatter().formatInBucketForm(crafting)));
        }
