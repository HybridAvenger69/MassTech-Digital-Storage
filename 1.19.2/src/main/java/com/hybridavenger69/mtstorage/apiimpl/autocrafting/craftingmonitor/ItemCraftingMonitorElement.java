...
public class ItemCraftingMonitorElement implements ICraftingMonitorElement {
    ...
    public void draw(PoseStack poseStack, int x, int y, IElementDrawers drawers) {
        ...
        if (stored > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.stored", stored));

            yy += 7;
        }

        if (missing > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.missing", missing));

            yy += 7;
        }

        if (processing > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.processing", processing));

            yy += 7;
        }

        if (scheduled > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.scheduled", scheduled));

            yy += 7;
        }

        if (crafting > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 25, scale), RenderUtils.getOffsetOnScale(yy, scale), I18n.get("gui.mtstorage.crafting_monitor.crafting", crafting));
        }
