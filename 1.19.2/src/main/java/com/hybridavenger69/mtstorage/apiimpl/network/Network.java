...
import com.hybridavenger69.mtstorage.api.network.INetworkNodeGraph;
...
import com.hybridavenger69.mtstorage.apiimpl.autocrafting.CraftingManager;
import com.hybridavenger69.mtstorage.apiimpl.network.grid.handler.FluidGridHandler;
import com.hybridavenger69.mtstorage.apiimpl.network.grid.handler.ItemGridHandler;
import com.hybridavenger69.mtstorage.apiimpl.network.item.NetworkItemManager;
import com.hybridavenger69.mtstorage.apiimpl.network.node.RootNetworkNode;
import com.hybridavenger69.mtstorage.apiimpl.network.security.SecurityManager;
...
import com.hybridavenger69.mtstorage.energy.BaseEnergyStorage;
...
public class Network implements INetwork, IRedstoneConfigurable {
    ...
    private final BaseEnergyStorage energy = new BaseEnergyStorage(MS.SERVER_CONFIG.getController().getCapacity(), MS.SERVER_CONFIG.getController().getMaxTransfer(), 0);
    ...
    public void update() {
        if (!level.isClientSide) {
            ...
            if (type == NetworkType.NORMAL) {
                if (!MS.SERVER_CONFIG.getController().getUseEnergy()) {
    ...
    private void updateEnergyUsage() {
        ...
        int usage = MS.SERVER_CONFIG.getController().getBaseUsage();
