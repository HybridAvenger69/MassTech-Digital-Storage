...
public class FluidStorageNetworkNode extends NetworkNode implements IStorageScreen, IStorageProvider, IComparable, IWhitelistBlacklist, IPrioritizable, IAccessType, IStorageDiskContainerContext {
    ...
    public int getEnergyUsage() {
        switch (type) {
            case SIXTY_FOUR_K:
                return MS.SERVER_CONFIG.getFluidStorageBlock().getSixtyFourKUsage();
            case TWO_HUNDRED_FIFTY_SIX_K:
                return MS.SERVER_CONFIG.getFluidStorageBlock().getTwoHundredFiftySixKUsage();
            case THOUSAND_TWENTY_FOUR_K:
                return MS.SERVER_CONFIG.getFluidStorageBlock().getThousandTwentyFourKUsage();
            case FOUR_THOUSAND_NINETY_SIX_K:
                return MS.SERVER_CONFIG.getFluidStorageBlock().getFourThousandNinetySixKUsage();
            case CREATIVE:
                return MS.SERVER_CONFIG.getFluidStorageBlock().getCreativeUsage();
            default:
                return 0;
        }
    }
    ...
    public Component getTitle() {
        return Component.translatable("block.mtstorage." + type.getName() + "_fluid_storage_block");
    }
