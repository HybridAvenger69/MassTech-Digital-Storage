package com.hybridavenger69.mtstorage.apiimpl.autocrafting.task.v6.monitor;
...
import com.hybridavenger69.mtstorage.apiimpl.autocrafting.task.v6.node.CraftingNode;
import com.hybridavenger69.mtstorage.apiimpl.autocrafting.task.v6.node.Node;
import com.hybridavenger69.mtstorage.apiimpl.autocrafting.task.v6.node.ProcessingNode;
import com.hybridavenger69.mtstorage.apiimpl.autocrafting.task.v6.node.ProcessingState;
...
public class CraftingMonitorElementFactory {
    ...
    private ICraftingMonitorElement wrapWithProcessingState(ICraftingMonitorElement element, ProcessingState state, String type) {
        if (state == ProcessingState.MACHINE_DOES_NOT_ACCEPT) {
            return new ErrorCraftingMonitorElement(element, "gui.mtstorage.crafting_monitor.machine_does_not_accept_" + type);
        } else if (state == ProcessingState.MACHINE_NONE) {
            return new ErrorCraftingMonitorElement(element, "gui.mtstorage.crafting_monitor.machine_none");
        } else if (state == ProcessingState.LOCKED) {
            return new ErrorCraftingMonitorElement(element, "gui.mtstorage.crafting_monitor.crafter_is_locked");
        }
