...
public class ItemCraftingPreviewElement implements ICraftingPreviewElement {
    ...
    public void draw(PoseStack poseStack, int x, int y, IElementDrawers drawers) {
        ...
        if (available > 0) {
            drawers.getStringDrawer().draw(poseStack, RenderUtils.getOffsetOnScale(x + 23, scale), RenderUtils.getOffsetOnScale(y, scale), I18n.get("gui.mtstorage.crafting_preview.available", available));
        }
