...
public class StorageScreen<T extends AbstractContainerMenu> extends BaseScreen<T> {
    ...
    public void onPostInit(int x, int y) {
        ...
        int buttonWidth = 10 + font.width(I18n.get("misc.mtstorage.priority"));

        addButton(
            x + 169 - buttonWidth,
            y + 41, buttonWidth,
            20,
            Component.translatable("misc.mtstorage.priority"),
            true,
            true,
    ...
    public void renderBackground(PoseStack poseStack, int x, int y, int mouseX, int mouseY) {
        bindTexture(HybridIDS.MTStorage_MODID, texture);
    ...
    public void renderForeground(PoseStack poseStack, int mouseX, int mouseY) {
        ...
        renderString(poseStack, 7, 42, capacitySupplier.get() == -1 ?
            I18n.get("misc.mtstorage.storage.stored_minimal", API.instance().getQuantityFormatter().formatWithUnits(storedSupplier.get())) :
            I18n.get("misc.mtstorage.storage.stored_capacity_minimal", API.instance().getQuantityFormatter().formatWithUnits(storedSupplier.get()), API.instance().getQuantityFormatter().formatWithUnits(capacitySupplier.get()))
        );
        ...
        if (RenderUtils.inBounds(BAR_X, BAR_Y, BAR_WIDTH, BAR_HEIGHT, mouseX, mouseY)) {
            ...
            renderTooltip(poseStack, mouseX, mouseY, (capacitySupplier.get() == -1 ?
                I18n.get("misc.mtstorage.storage.stored_minimal", API.instance().getQuantityFormatter().format(storedSupplier.get())) :
                I18n.get("misc.mtstorage.storage.stored_capacity_minimal", API.instance().getQuantityFormatter().format(storedSupplier.get()), API.instance().getQuantityFormatter().format(capacitySupplier.get()))
            ) + "\n" + ChatFormatting.GRAY + I18n.get("misc.mtstorage.storage.full", full));
        }
    }
}
