...
import com.hybridavenger69.mtstorage.network.SecurityManagerUpdateMessage;
...
public class SecurityManagerScreen extends BaseScreen<SecurityManagerContainerMenu> {
    ...
    public void onPostInit(int x, int y) {
        ...
        permissions[0] = addCheckBox(x + 7, y + 93, Component.translatable("gui.mtstorage.security_manager.permission.0"), false, btn -> handle(0));
        permissions[1] = addCheckBox(permissions[0].x, permissions[0].y + padding, Component.translatable("gui.mtstorage.security_manager.permission.1"), false, btn -> handle(1));
        permissions[2] = addCheckBox(permissions[1].x, permissions[1].y + padding, Component.translatable("gui.mtstorage.security_manager.permission.2"), false, btn -> handle(2));
        permissions[3] = addCheckBox(permissions[0].x + 90, permissions[0].y, Component.translatable("gui.mtstorage.security_manager.permission.3"), false, btn -> handle(3));
        permissions[4] = addCheckBox(permissions[3].x, permissions[3].y + padding, Component.translatable("gui.mtstorage.security_manager.permission.4"), false, btn -> handle(4));
        permissions[5] = addCheckBox(permissions[4].x, permissions[4].y + padding, Component.translatable("gui.mtstorage.security_manager.permission.5"), false, btn -> handle(5));
    }

    private void handle(int i) {
        MS.NETWORK_HANDLER.sendToServer(new SecurityManagerUpdateMessage(securityManager.getBlockPos(), Permission.values()[i], permissions[i].selected()));
    }
    ...
    public void renderForeground(PoseStack poseStack, int mouseX, int mouseY) {
        ...
        renderString(poseStack, 7, 59, I18n.get("gui.mtstorage.security_manager.configure"));
        ...
        for (int i = 0; i < permissions.length; ++i) {
            ...
            if (RenderUtils.inBounds(permission.x - leftPos, permission.y - topPos, permission.getWidth(), permission.getHeight(), mouseX, mouseY)) {
                renderTooltip(poseStack, mouseX, mouseY, I18n.get("gui.mtstorage.security_manager.permission." + i + ".tooltip"));
            }
        }
    }
}
