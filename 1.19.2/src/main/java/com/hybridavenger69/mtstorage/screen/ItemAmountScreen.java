...
import com.hybridavenger69.mtstorage.container.AmountContainerMenu;
...
public class ItemAmountScreen extends AmountSpecifyingScreen<AmountContainerMenu> {
    ...
    public ItemAmountScreen(BaseScreen parent, Player player, int containerSlot, ItemStack stack, int maxAmount, @Nullable UnaryOperator<Screen> alternativesScreenFactory) {
        super(parent, new AmountContainerMenu(player, stack), alternativesScreenFactory != null ? 194 : 172, 99, player.getInventory(), Component.translatable("gui.mtstorage.item_amount"));
    ...
    public void onPostInit(int x, int y) {
        ...
        if (alternativesScreenFactory != null) {
            addButton(x + 114, cancelButton.y + 24, getOkCancelButtonWidth(), 20, Component.translatable("gui.mtstorage.alternatives"), true, true, btn -> minecraft.setScreen(alternativesScreenFactory.apply(this)));
        }
    }
    ...
    protected Component getOkButtonText() {
        return Component.translatable("misc.mtstorage.set");
    }
    ...
    protected void onOkButtonPressed(boolean shiftDown) {
        try {
            ...
            MS.NETWORK_HANDLER.sendToServer(new SetFilterSlotMessage(containerSlot, ItemHandlerHelper.copyStackWithSize(stack, amount)));
