...
public class RecipeTransferCraftingGridError implements IRecipeTransferError {
    ...
    protected List<Component> drawIngredientHighlights(PoseStack stack, int recipeX, int recipeY) {
        ...
        if (craftMessage) {
            if (HOST_OS_IS_MACOS) {
                message.add(Component.translatable("gui.mtstorage.jei.transfer.request_autocrafting_mac").withStyle(ChatFormatting.BLUE));
            } else {
                message.add(Component.translatable("gui.mtstorage.jei.transfer.request_autocrafting").withStyle(ChatFormatting.BLUE));
            }
        }
